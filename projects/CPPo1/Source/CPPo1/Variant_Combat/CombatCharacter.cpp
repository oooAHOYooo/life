...
#include "CPPo1LockOnTargetComponent.h"
...
ACombatCharacter::ACombatCharacter()
{
	...
	// Third-person action style control: move-facing character with camera-driven aim.
...
void ACombatCharacter::DoLockOn()
{
	...
	UCPPo1LockOnTargetComponent::FindLockOnTargetsInRadius(GetWorld(), GetActorLocation(), LockOnRadius, Targets);
...
void ACombatCharacter::BeginPlay()
{
	...
	if (LifeBar)
	{
		...
		if (!LifeBarWidget)
		{
			UE_LOG(LogCombatCharacter, Warning, TEXT("LifeBar widget instance is missing or wrong class on %s. Continuing without life bar UI."), *GetName());
		}
	}
	else
	{
		UE_LOG(LogCombatCharacter, Warning, TEXT("LifeBar component is null on %s. Continuing without life bar UI."), *GetName());
	}
...
void ACombatCharacter::Tick(float DeltaTime)
{
	...
	{
		UCPPo1LockOnTargetComponent* Comp = LockOnTarget->FindComponentByClass<UCPPo1LockOnTargetComponent>();
		if (Comp)
		{
